...
def get_full_page_source_selenium(url):
    ...
    while True:
        try:
            ...
            load_more_button = driver.find_element(By.CSS_SELECTOR, "a[title='Load more items']")
...
def extract_conference_details(article, base_url):
    ...
    name_link_tag = article.find('h3', class_='field--node--field-display-title').find('a')
    if name_link_tag:
    ...
    if dates_tag and len(dates_tag.find_all('span')) > 1:
        details['dates'] = dates_tag.find_all('span')[1].get_text(strip=True, separator=' ')
    else:
        details['dates'] = None
...
def main():
    ...
    for article in all_conference_articles:
        ...
        if conference_info.get('shortform'): # Only add if it's a valid entry with a shortform
...
if __name__ == "__main__":
    ...
    for conf in all_data:
        print(f"Shortform: {conf.get('shortform')}, Name: {conf.get('name')}, Link: {conf.get('link')}, "
